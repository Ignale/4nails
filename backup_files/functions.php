...
/*  Создание critical CSS для разных страниц  */
require_once "app/critical_css_rules.php";
...
function getYoutubeTitle($video_id)
{
  ...
  */
  ...
  // Check if there is data in the cache(transient)
  $transient_key = 'youtube_title_' . $video_id;
  ...
  // If there is no data in the cache, we carry out a request to YouTube API
  $api_key = "AIzaSyCn3U5crNIAyod5qzr1OMvJJnRFs3KL9Ws";
  $url = "https://www.googleapis.com/youtube/v3/videos?id=" . $video_id . "&key=" . $api_key . "&part=snippet,contentDetails,statistics,status";
  $json = file_get_contents($url);
  $getData = json_decode($json, true);
  ...
  if ($title) {
    // We keep the result in the cache for 24 hours (86400 seconds)
...
function save_comment_wpse_82317($location, $comment_id)
{
  ...
  if (
    !wp_verify_nonce($_POST['noncename_wpse_82317'], plugin_basename(__FILE__))
