...
<div class="checkout__order-item">
  ...
  <?php foreach ($available_methods as $method):
              ...
              if (if_overweight(WC()->cart) && $method->id !== 'overweight_shipping' && $method->method_id !== 'local_pickup') {
                  continue;
              }
          ?>
					    <li>
					      <?php
                                  if (1 <= count($available_methods)) {
                                          ...
                                          printf('<label class="custom-control-label" for="shipping_method_%1$s_%2$s">%3$s</label></div>', $index, esc_attr(sanitize_title($method->id)), add_shipping_info($method, $chosen_method)); //   $chosen_method = 'overweight_shipping';
                                      }
