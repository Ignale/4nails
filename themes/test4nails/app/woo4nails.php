...
*/
    ...
    function nails_get_totals($order_id = 0)
    {
        ...
        if (is_admin()) {
            ...
            foreach ($order->get_items() as $item) {
                ...
                $sale_total += ($product->regular_price - $price) * $quantity;
            }
    ...
    */
    ...
    {
        preg_match('/(\d|\d*-\d*)(?= working days)/', $method->label, $working_days_match);

        $working_days = $working_days_match[0] ?: null;

        preg_match('/(?<=USD )\d*.\d*/', $method->get_label(), $taxes_match);

        $taxes = $taxes_match[0] ?: null;

        $method_label = $method->get_label();

        $method_label = __($method_label, '4nails');

        if (preg_match('/ DHL Express - Worldwide /', $method->get_label())) {
            $method_label = sprintf(__('DHL Express - Worldwide (%s working days)<div class="shipping_method-description"> No additional taxes to be paid at delivery</div>', '4nails'
            ), $working_days ?: '5');
            if ($taxes) {
                $method_label = sprintf(__('DHL Express - Worldwide (%s working days)<div class="shipping_method-description">  Estimated USD %s tax & duty due on delivery (Tax handling fees may apply</div>', '4nails'
                ), $working_days ?: '5', $taxes ?: '0');
            }
        }
        if (preg_match('/First Class International/', $method->get_label())) {
            $method_label = sprintf(__('USPS - First Class International (%s working days)<div class="shipping_method-description"> No additional taxes to be paid at delivery</div>', '4nails'), $working_days ?: '5');
            if ($taxes) {
                $method_label = sprintf(__('USPS - First Class International (%s working days)<div class="shipping_method-description">  Estimated USD %s tax & duty due on delivery (Tax handling fees may apply</div>',
                    '4nails'
                ), $working_days ?: '5', $taxes ?: '0');
            }
        }

        if (preg_match('/Priority Mail International/', $method->get_label())) {
            $method_label = sprintf(__('USPS - Priority Mail International (%s working days)<div class="shipping_method-description"> No additional taxes to be paid at delivery</div>', '4nails'), $working_days ?: '5');
            if ($taxes) {
                $method_label = sprintf(__('USPS - Priority Mail International (%s working days)<div class="shipping_method-description">  Estimated USD %s tax & duty due on delivery</div>',
                    '4nails'
                ), $working_days ?: '5', $taxes ?: '0');
            }
        }
        ...
        $mark    = '';
        $mark .= "<div data-temp='" . get_shipping_label_id($method->label) . '_info' . "'>";

        if ($method->cost == 0) {
            if ($method->id == 'overweight_shipping') {
                $mark .= "<div class='delivery__info-text' >" . get_field('overweight_shipping_info', 'options');
            }
        ...
        if (get_shipping_label_id($method->label) == 'express_mail') {
            if (date('w') == 6) {
                $d = strtotime("+2 day");
            } else {
                $d = strtotime("+1 day");
  ...
  <style>
    ...
    .woocommerce .loader:before {
      ...
      margin-top: 0;
      display: block;
      content: "<?php echo __('Please Wait', '4nails'); ?> ...";
    ...
    .woocommerce .loader:before {
      content: "<?php echo __('Thank you for the answers', '4nails'); ?> ...";
    }
...
<?php
    ...
    function apply_fee($acf_option, $fee_percent, $total_price, $fee_name)
    {
        ...
        // write_log_info("Payment method: $fee_name, total price: $total_price");

        // var_dump($acf_option, $fee_percent, $total_price, $fee_name);
    ...
    function fg_add_fee()
    {
        ...
        if ($customer_billing_address !== "US") {

            switch ($payment_method) {
                case "paypal":
                    apply_fee('international_shipping_paypal', 0.06, $total_price, __('PayPal fee', '4nails'));
                    break;
                case "stripe":
                    apply_fee('international_shipping_stripe', 0.05, $total_price, __('Stripe fee', '4nails'));
                    break;
                case "affirm":
                    apply_fee('affirm_fee', 0.06, $total_price, __('Affirm fee', '4nails'));
                    break;
                case "zelle":
                    apply_fee('zelle_fee', 0, $total_price, __('Zelle fee', '4nails'));
                    break;
            }
            return;
        }
        ...
        switch ($payment_method) {
            case "paypal":
                apply_fee('usa_shipping_paypal', 0.033, $total_price, __('PayPal fee', '4nails'));
                break;
            case "stripe":
                apply_fee('usa_shipping_stripe', 0.03, $total_price, __('Stripe fee', '4nails'));
                break;
            case "affirm":
                apply_fee('affirm_fee', 0.06, $total_price, __('Affirm fee', '4nails'));
                break;
    ...
    function action_mark_order_as_cancel_request()
    {
        ...
        if (! is_user_logged_in()) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wc-cancel-order'), '', ['response' => 403]);
        }

        if (! check_admin_referer('woocommerce-mark-order-cancell-request-myaccount')) {
            wp_die(__('You have taken too long. Please go back and retry.', 'wc-cancel-order'), '', ['response' => 403]);
        }
    ...
    function get_tracking_order($order)
    {
        ...
        if (! empty($labels)) {
            ...
            return ['url' => $tracking_url, 'number' => $tracking];
        }
    }
    ...
    */
    ...
    function change_shipping_methods_label_names($rates, $package)
    {
        ...
        foreach ($rates as $rate_key => $rate) {
            ...
            $rate->label = __($rate->label, '4nails'); // New label name
...
<?php endif;
    ...
    //Сохраняет цену товара в мету при создании заказа
    add_action('woocommerce_checkout_order_created', 'nails_save_discount_price', 10, 4);

    //показываем все произвольные пользовательские поля в заказе
    // add_filter('acf/settings/remove_wp_meta_box', '__return_false');

    function nails_save_discount_price($order)
    {
        ...
        foreach ($items as $item) {
            ...
            // Сохраняем цену товара в мета
            $product_id = $item->get_product_id();

            $regualar_price = wc_get_product($product_id)->get_regular_price();
            $line_subtotal  = round(get_product_price($product_id), 2);

            $line_sale_amount = ($regualar_price - $line_subtotal) / $regualar_price * 100;

            update_post_meta($item->get_order_id(), $product_id . '_regular_price', $regualar_price, true);

            update_post_meta($item->get_order_id(), $product_id . '_price', $line_subtotal, true);

            update_post_meta($item->get_order_id(), $product_id . '_sale_amount', $line_sale_amount, true);

            $item->set_subtotal($line_subtotal * $item['quantity']);

            $item->set_total($line_subtotal * $item['quantity']);
        }

    }

    //Выводит цену товара без учета скидки и процент скидки в админке на странице заказа
    ...
    function my_woocommerce_admin_order_item_values($_product, $item, $item_id = null)
    {
        ...
        if (isset($_product) && isset($_product->id)) {
            ...
            if ($item['type'] == "line_item") {
                ...
                $product_id = apply_filters('wpml_object_id', $_product->get_id(), 'product', false, get_post_meta($order_id, 'wpml_language', true) ?: 'en');

                $order = wc_get_order($order_id);
                // $meta = wc_get_order_item_meta($item_id, '_product_price', true);
                $actual = get_post_meta($order_id, $product_id . '_regualar_price', true);

                if (! $actual) {
                    $actual = $_product->get_regular_price();
                }

                $sale_amount = get_post_meta($order_id, $product_id . '_sale_amount', true);
    ...
    function get_order_item_subtotal($item, $order)
    {
        $product_id = apply_filters('wpml_object_id', $item['product_id'], 'product', false, get_post_meta($order->get_id(), 'wpml_language', true) ?: 'en');
        // Если цена товара была сохранена в мета, то берем ее оттуда
        $subtotal = get_post_meta($order->get_id(), $product_id . '_price', true);

        if (! $subtotal) {
            //если цена не найдена в мета, смотрим текущую цену
    ...
    //Выводит цену товара в заказе с учетом скидки
    add_filter('woocommerce_order_amount_item_subtotal', 'nails_woocommerce_order_amount_item_sale_price', 10, 5);
    function nails_woocommerce_order_amount_item_sale_price($subtotal, $order, $item, $inc_tax, $round)
    {

        $subtotal = get_order_item_subtotal($item, $order);

        $item->set_total($subtotal * $item['quantity']);

        $item->set_subtotal($subtotal * $item['quantity']);
    ...
    function nails_invoice_values($row, $item_id, $item, $invoice)
    {
        $product_id = apply_filters('wpml_object_id', $item['product_id'], 'product', false, get_post_meta($item['order_id'], 'wpml_language', true) ?: 'en');
        $order_id   = $item['order_id'];
        $product    = wc_get_product($product_id);

        $regular_price = get_post_meta($order_id, $product_id . '_regualar_price', true);

        $product_price = get_post_meta($order_id, $product_id . '_price', true);
        ...
        if (! $product_price) {
            $product_price = get_product_price($product_id, $order_id);
        }
        $sale_amount = get_post_meta($order_id, $product_id . '_sale_amount', true);

        if (! $sale_amount) {
            $sale_amount = ($regular_price - $product_price) / $regular_price * 100;
        }
        ...
        $row = array_insert($row, 3, ['sale_price' => wc_price($product_price)]);
        ...
        $row['total'] = wc_price($total_price);

        return $row;
    }

    add_filter('wpi_get_invoice_columns_data_row', 'nails_invoice_values', 20, 4);
    ...
    function show_total_price_in_admin_menu(): void
    {
        $screen    = get_current_screen();
        $post_type = get_post_type();
        if (is_admin() && $screen->action == '' && $screen->base == 'edit' && $post_type == 'product') {

            echo "<div id='goods-info' style=' position: absolute;top: 73px;left: 650px;'>" . "<h2 style='color: red'>" . 'Total price of products: ' . show_summ_all_products($_GET['product_cat']) . '</h2>' . '</div>';
        }
    }
    ...
    function get_taxes_total($cart, $total = 0)
    {
        ...
        $rate = $rates ? reset($rates)['rate'] : null;
    ...
    add_action('woocommerce_checkout_update_order_review', function ($posted_data) {
        ...
        // если выбран Stripe — перезапускаем расчёт доставки
        if (! empty($output['payment_method']) && str_contains($output['payment_method'], 'stripe')) {
            // заставляем WooCommerce заново рассчитать тарифы
